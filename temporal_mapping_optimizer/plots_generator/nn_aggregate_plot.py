...
for i in range(0, 360, 10):
    ...
    for layer_idx in layer_range:
        if layer_idx in duplicate_layer_idx_dict.keys():
            ...
            mcmc_ut_sum += mcmc_net_perf_list[duplicate_layer_idx_dict[layer_idx]][1]
            mcmc_lat_sum += mcmc_net_perf_list[duplicate_layer_idx_dict[layer_idx]][2]
            mcmc_time_sum += mcmc_net_perf_list[duplicate_layer_idx_dict[layer_idx]][3]

            loma_en_sum += loma_net_perf_list[duplicate_layer_idx_dict[layer_idx]][0]
            loma_ut_sum += loma_net_perf_list[duplicate_layer_idx_dict[layer_idx]][1]
            loma_lat_sum += loma_net_perf_list[duplicate_layer_idx_dict[layer_idx]][2]
            loma_time_sum += loma_net_perf_list[duplicate_layer_idx_dict[layer_idx]][3]
        else:
            mcmc_net_perf_list.append([data_doc[layer_idx]['mcmc']['en'], data_doc[layer_idx]['mcmc']['ut'], data_doc[layer_idx]['mcmc']['lat'], data_doc[layer_idx]['mcmc']['exec_time']])
            loma_net_perf_list.append([data_doc[layer_idx]['loma']['en'], data_doc[layer_idx]['loma']['ut'], data_doc[layer_idx]['loma']['lat'], data_doc[layer_idx]['loma']['exec_time']])
            
            mcmc_en_sum += data_doc[layer_idx]['mcmc']['en']
            mcmc_ut_sum += data_doc[layer_idx]['mcmc']['ut']
            ...
            loma_ut_sum += data_doc[layer_idx]['loma']['ut']
    ...
    mcmc_arch_ut_avg_list.append(mcmc_ut_sum / network_depth)
    ...
    loma_arch_ut_avg_list.append(loma_ut_sum / network_depth)
...
arch_range_mcmc = [x - 0.25 for x in arch_range]
...
axs[0, 1].set_title(nn_name + " Utilization")
axs[0, 2].set_title(nn_name + " Latency")
axs[1, 2].set_title(nn_name + " Time")

axs[0, 0].bar([0], statistics.mean(mcmc_arch_en_avg_list), label='mcmc', color='tab:orange', width = 0.5, alpha=0.66, linewidth=2)
axs[0, 0].bar([1], statistics.mean(loma_arch_en_avg_list), label='loma', color='tab:blue', width = 0.5, alpha=0.66, linewidth=2)
axs[0, 0].set_ylabel("Energy")

axs[0, 1].bar([0], statistics.mean(mcmc_arch_ut_avg_list), label='mcmc', color='tab:orange', width = 0.5, alpha=0.66, linewidth=2)
axs[0, 1].bar([1], statistics.mean(loma_arch_ut_avg_list), label='loma', color='tab:blue', width = 0.5, alpha=0.66, linewidth=2)
axs[0, 1].set_ylabel("Utilization")

axs[0, 2].bar([0], statistics.mean(mcmc_arch_lat_avg_list), label='mcmc', color='tab:orange', width = 0.5, alpha=0.66, linewidth=2)
axs[0, 2].bar([1], statistics.mean(loma_arch_lat_avg_list), label='loma', color='tab:blue', width = 0.5, alpha=0.66, linewidth=2)
axs[0, 2].set_ylabel("Latency (cycles)")

axs[1, 2].bar([0], statistics.mean(mcmc_arch_time_avg_list), label='mcmc', color='tab:orange', width = 0.5, alpha=0.66, linewidth=2)
axs[1, 2].bar([1], statistics.mean(loma_arch_time_avg_list), label='loma', color='tab:blue', width = 0.5, alpha=0.66, linewidth=2)
axs[1, 2].set_ylabel("Time (s)")

axs[1, 0].bar(arch_range_mcmc, mcmc_arch_en_avg_list, label='mcmc', color='tab:orange', width = 0.5, alpha=0.66, linewidth=2)
axs[1, 0].bar(arch_range_loma, loma_arch_en_avg_list, label='loma', color='tab:blue', width = 0.5, alpha=0.66, linewidth=2)
axs[1, 0].set_xlabel("Arch")
axs[1, 0].set_ylabel("Energy")

axs[1, 1].bar(arch_range_mcmc, mcmc_arch_ut_avg_list, label='mcmc', color='tab:orange', width = 0.5, alpha=0.66, linewidth=2)
axs[1, 1].bar(arch_range_loma, loma_arch_ut_avg_list, label='loma', color='tab:blue', width = 0.5, alpha=0.66, linewidth=2)
