...
tl_to_tl_time_list = [86,468,224,343,254,304,181,195]
...
zz_to_tl_energy_sum = [sum(x) for x in zip(zz_to_tl_w_cost, zz_to_tl_i_cost, zz_to_tl_o_cost, zz_to_tl_buff_cost, zz_to_tl_dram_cost)]

print(zz_to_zz_time_list)
print(tl_to_tl_energy_sum)
print(zz_to_tl_energy_sum)
...
ax[0].bar(X+(0.20), [sum(x) for x in zip(zz_to_tl_w_cost, zz_to_tl_i_cost, zz_to_tl_o_cost, zz_to_tl_buff_cost, zz_to_tl_dram_cost)], width = w, label='DRAM', color="#e76f51", linewidth=1, edgecolor='grey')
ax[0].bar(X+(0.20), [sum(x) for x in zip(zz_to_tl_w_cost, zz_to_tl_i_cost, zz_to_tl_o_cost, zz_to_tl_buff_cost)], width = w, label='shared_buff', color="#f4a261", linewidth=1, edgecolor='grey')
ax[0].bar(X+(0.20), [sum(x) for x in zip(zz_to_tl_w_cost, zz_to_tl_i_cost, zz_to_tl_o_cost)], width = w, label='O-reg', color="#e9c46a", linewidth=1, edgecolor='grey')
ax[0].bar(X+(0.20), [sum(x) for x in zip(zz_to_tl_w_cost, zz_to_tl_i_cost)], width = w, label='I-reg', color="#2a9d8f", linewidth=1, edgecolor='grey')
ax[0].bar(X+(0.20), zz_to_tl_w_cost, width = w, label='W-reg', color="#264653", linewidth=1, edgecolor='grey')
...
for i in range(number_of_layer):
    ...
    ax[1].annotate('Timeloop', xy=(i*offset + 0.060, -0.365), xycoords='axes fraction', fontsize=19, rotation=-75)
    ax[1].annotate('SALSA', xy=(i*offset + 0.105, -0.265), xycoords='axes fraction', fontsize=19, rotation=-75)
...
# we use the time of zz to zz because the mapping has been found by zigzag and just injected into timeloop
